...
class ServerData:
    ...
    def from_dict(obj: dict) -> 'ServerData':
        ...
        return ServerData(_ip, _map, _players, _sleepers, _maxplayers, _queue, _joining, _time, _server, _wipeday, _gm, _limit, _lastupdate, _num)
...
def get_servers_data() -> list[ServerData]:
    ...
    servers = [ServerData.from_dict(server_data) for server_data in data_dict.values() \
               if server_data['lastupdate'] <= SERVER_LASTUPDATE_TRESHOLD and \
                server_data.get("gm", None) != "test"]
    return servers
